...
public class ScrapScript : MonoBehaviour
{
    ...
    {
        if (collision.transform.CompareTag("Box") || collision.transform.CompareTag("FirstGround") || collision.transform.CompareTag("Panda") || collision.transform.CompareTag("Bamboo")) //On vérifie si le tag de l'objet rencontré est celui d'un objet faisant exploser le débris
        {
            ...
            foreach(Collider2D collider in hitByExplosion) //On parcours tous les éléments de la liste de Collider2D
            {
                ...
                if(hitGameObject.transform.CompareTag("Bamboo"))
                {
                    bamboo = hitGameObject.GetComponent<bambooGrow>();
                    bamboo.bambooHealth -= 1;
                }
            }
        }
    }
